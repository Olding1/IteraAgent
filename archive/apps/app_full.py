"""
Agent Zero v8.0 - 完整 Streamlit UI
...
with st.sidebar:
    ...
    # Navigation
    page = st.radio("导航", ["🏠 首页", "📦 Agent 管理", "📤 导出功能", "⚙️ 设置"], key="navigation")
...
if page == "🏠 首页":
    ...
    with col1:
        ...
        if env_file.exists():
            ...
            st.error("❌ .env 文件不存在")
            st.info("请从 .env.template 创建 .env 文件")
    ...
    with col1:
        if st.button("📦 管理 Agent", use_container_width=True, type="primary"):
            st.session_state.current_page = "agent_management"
            st.rerun()

    with col2:
        if st.button("📤 导出到 Dify", use_container_width=True):
    ...
    for agent in agents:
        with st.expander(f"📁 {agent.name}"):
            ...
            with col2:
                ...
                if st.button("📊 详情", key=f"detail_{agent.name}", use_container_width=True):
                    st.session_state.selected_agent = agent.name
                    st.info("详情功能开发中...")
...
# ============================================================
    ...
    # Pre-select if coming from agent management
    default_index = 0
...
st.markdown(
    ...
    "🤖 Agent Zero v8.0 Phase 5 | Built with ❤️ using Streamlit"
    "</div>",
