...
async def check_system_health():
    ...
    print(
        f"   {t('api_key')}: {t('api_key_configured') if builder_config.api_key else t('api_key_missing')}"
    )
    ...
    print(
        f"   {t('api_key')}: {t('api_key_configured') if runtime_config.api_key else t('api_key_missing')}"
    )

    # Perform health checks
    print(f"\n{t('testing_connectivity')}")
    try:
        builder_result, runtime_result = await check_all_apis(builder_config, runtime_config)
                ...
                if agents:
                    ...
                    try:
                        ...
                        if idx > 0 and idx <= len(agents):
                            ...
                            if action == "1":
                                # Run python agent.py
                                install_script = target_agent / (
                                    "install.bat" if os.name == "nt" else "install.sh"
                                )
                                ...
                                try:
                                    ...
                                    subprocess.run(
                                        [str(venv_python), str(agent_script)], cwd=str(target_agent)
                                    )
                                except Exception as e:
                                    print(f"执行出错: {e}")
                    ...
                    try:
                        ...
                        if 1 <= idx <= len(agents):
                            ...
                            for iteration in range(
                                start_iteration, start_iteration + max_iterations
                            ):
                                print("\n" + "=" * 70)
                                print(
                                    f"🚀 开始迭代 {iteration} ({'自动模式' if auto_mode else '手动模式'})"
                                )
                                print("=" * 70)
                                ...
                                try:
                                    ...
                                    if (
                                        ...
                                        from src.schemas.execution_result import (
                                            ExecutionStatus,
                                            ExecutionResult,
                                            TestResult,
                                        )
                                            ...
                                            test_results=[
                                                TestResult(
                                                    ...
                                                    status=(
                                                        ...
                                                        if tc.status.upper() in ["FAIL", "FAILED"]
                                                        else ExecutionStatus.PASS
                                                    ),
                                                    actual_output=tc.actual_output,
                                                    error_message=tc.error_message,
                                                    duration_ms=(
                                                        int(tc.duration_seconds * 1000)
                                                        if hasattr(tc, "duration_seconds")
                                                        else 0
                                                    ),
                                                )
                                        ...
                                        # 2. Create report
                                        from src.schemas.test_report import (
                                            TestCaseReport,
                                            IterationReport,
                                        )
                                            ...
                                            test_cases.append(
                                                TestCaseReport(
                                                    ...
                                                    status=(
                                                        ...
                                                        if hasattr(test.status, "value")
                                                        else str(test.status).upper()
                                                    ),
                                        ...
                                        passed_tests = sum(
                                            ...
                                            if t.status.value in ["pass", "success"]
                                        )
                                        ...
                                        iteration_report = IterationReport(
                                            ...
                                            rag_config_snapshot=(
                                                rag_config.model_dump() if rag_config else None
                                            ),
                                            tools_config_snapshot=(
                                                tools_config.model_dump() if tools_config else None
                                            ),
                                        )
                                    ...
                                    if test_results.overall_status != ExecutionStatus.SKIPPED:
                                        ...
                                        iteration_report.fix_target = (
                                            judge_result.fix_target.value
                                            if judge_result.fix_target
                                            else None
                                        )
                                        ...
                                        if judge_result.error_type != "none":
                                            ...
                                            try:
                                                ...
                                                callback.on_log(f"\n🔍 [DEBUG] 修复策略详情:")
                                                callback.on_log(
                                                    f"  策略总数: {len(analysis.fix_strategy)}"
                                                )
                                                for i, step in enumerate(analysis.fix_strategy, 1):
                                                    callback.on_log(
                                                        f"  策略 {i}: target={step.target}, priority={step.priority}"
                                                    )
                                                    callback.on_log(
                                                        f"         action={step.action[:80]}..."
                                                    )
                                            except Exception as e:
                                            ...
                                            iteration_report.judge_feedback = (
                                                judge_result.feedback if judge_result else ""
                                            )
                                    ...
                                    if analysis and analysis.fix_strategy:
                                        ...
                                        for fix_step in analysis.fix_strategy[:3]:
                                            ...
                                            callback.on_log(
                                                f"     [DEBUG] target={fix_step.target}, priority={fix_step.priority}"
                                            )

                                            try:
                                                if fix_step.target == "rag_builder" and rag_config:
                                                    callback.on_log(
                                                        f"     [DEBUG] 进入 RAGOptimizer 分支"
                                                    )
                                                    ...
                                                    new_rag_config = (
                                                        await rag_optimizer.optimize_config(
                                                    ...
                                                    rag_config_file = (
                                                        target_agent / "rag_config.json"
                                                    )
                                                    with open(
                                                        rag_config_file, "w", encoding="utf-8"
                                                    ) as f:
                                                        json.dump(
                                                    ...
                                                    fix_step.target == "rag_builder"
                                                    and not rag_config
                                                ):
                                                    callback.on_log(
                                                        f"     [DEBUG] 跳过 RAGOptimizer: rag_config 不存在"
                                                    )

                                                elif (
                                                    fix_step.target == "tool_selector"
                                                    and tools_config
                                                ):
                                                    callback.on_log(
                                                        f"     [DEBUG] 进入 ToolOptimizer 分支"
                                                    )
                                                    from src.core.tool_optimizer import (
                                                        ToolOptimizer,
                                                    )

                                                    tool_optimizer = ToolOptimizer(
                                                        builder_client, tool_selector
                                                    )

                                                    new_tools_config = (
                                                        await tool_optimizer.optimize_tools(
                                                    ...
                                                    callback.on_log(
                                                        f"     [DEBUG] 跳过 ToolOptimizer: tools_config 不存在"
                                                    )
                                                    compiler.compile(
                                                        meta,
                                                        graph,
                                                    ...
                                                    callback.on_log(
                                                        f"     [DEBUG] 进入 GraphOptimizer 分支"
                                                    )
                                                    from src.core.graph_optimizer import (
                                                        GraphOptimizer,
                                                    )
                                                    ...
                                                    new_graph, sim_result = (
                                                        await graph_optimizer.optimize_graph(
                                                            graph, analysis, meta
                                                        )
                                                    )
                                                    ...
                                                    callback.on_log(
                                                        f"     [DEBUG] 进入 CompilerOptimizer 分支"
                                                    )
                                                    from src.core.compiler_optimizer import (
                                                        CompilerOptimizer,
                                                    )
                                                    ...
                                                    success = await compiler_optimizer.optimize_dependencies(
                                                        target_agent, analysis, error_msg
                                                    )

                                                    if success:
                                                        callback.on_log(
                                                            f"    ✅ Compiler 优化: 已更新依赖项"
                                                        )
                                                    ...
                                                    callback.on_log(
                                                        f"     [DEBUG] target='{fix_step.target}', rag_config={rag_config is not None}, tools_config={tools_config is not None}"
                                                    )
                                                ...
                                                callback.on_log(
                                                    f"    [DEBUG] 异常详情: {traceback.format_exc()[:200]}"
                                                )
                        ...
                        if 1 <= idx <= len(agents):
                            ...
                            if export_choice in ["1", "2", "3"]:
                                ...
                                if any(node.type == "rag" for node in graph.nodes):
                                    print(
                                        "   4. 手动添加 Knowledge Retrieval 节点（RAG 节点已跳过）"
                                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--lang", choices=["zh", "en"], help="Set language: zh (Chinese) or en (English)"
    )
