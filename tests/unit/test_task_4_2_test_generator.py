...
from src.core.test_generator import TestGenerator as CoreTestGenerator, DeepEvalTestConfig
...
def test_generate_imports():
    ...
    generator = CoreTestGenerator(MockLLMClient())
    config = DeepEvalTestConfig()
...
def test_generate_deepeval_config_optimized():
    ...
    # 验证使用了 IteraAgentJudge 适配器类
    assert "class IteraAgentJudge" in config_code
