...
async def test_real_rag():
    ...
    try:
        project_meta = await pm.analyze_requirements(
            user_input="创建一个能回答 IteraAgent 项目相关问题的智能助手,可以查询项目架构、实施计划和技术细节",
        ...
        project_meta = await pm.analyze_requirements(
            user_input="创建一个能回答 IteraAgent 项目相关问题的智能助手", file_paths=existing_files
        )
