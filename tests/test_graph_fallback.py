...
async def test_sequential_pattern_fallback():
    ...
    has_tool_return = any(e.source == "tool_tavily_search" and e.target == agent_nodes[0].id for e in graph.edges)
    ...
    for e in graph.conditional_edges:
        if e.source == agent_nodes[0].id:
            ...
            if "tavily_search" in e.branches and e.branches["tavily_search"] == "tool_tavily_search":
                has_agent_out = True
                break
