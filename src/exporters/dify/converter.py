"""
IteraAgent -> Dify DSL 转换器

将 IteraAgent 的 Graph 结构转换为 Dify 的 YAML 格式
"""
...
class IteraAgentToDifyConverter:
    """IteraAgent -> Dify DSL 转换器"""
    ...
    def __init__(self, graph: GraphStructure, agent_name: str):
        ...
        Args:
            graph: IteraAgent Graph 结构
            agent_name: Agent 名称
        """
        self.graph = graph
        self.agent_name = agent_name
        self.node_id_counter = 1
        self.node_id_map = {}  # IteraAgent node.id -> Dify node id
    ...
    def convert(self) -> DifyApp:
        ...
        app = DifyApp(
            app={
                ...
                or f"{self.agent_name} - Generated by IteraAgent",
    ...
    def _create_nodes(self) -> List[DifyNode]:
        ...
        for node in self.graph.nodes:
            # 跳过 RAG 节点（Dify 需要手动配置知识库）
            if node.type == "rag":
                print(
                    f"⚠️  跳过 RAG 节点 '{node.id}'（需要在 Dify 中手动添加 Knowledge Retrieval 节点）"
                )
                # 不分配 node_id，这样引用此节点的边也会被跳过
                continue
    ...
    def _create_start_node(self) -> DifyNode:
        ...
        # 映射 State 字段为 Start 节点 Variables
        variables = [
            {"variable": "query", "type": "text-input", "label": "用户输入", "required": True}
        ]
        ...
        return DifyNode(
            id=start_id,
            data=DifyNodeData(title="开始", type="start", variables=variables),
            position={"x": 0, "y": 100},
        )
    ...
    def _create_answer_node(self, index: int) -> DifyNode:
        ...
        return DifyNode(
            id=answer_id,
            data=DifyNodeData(
                title="回答", type="answer", answer="{{#llm.text#}}"  # 引用最后一个 LLM 节点的输出
            ),
    ...
    def _create_edges(self, nodes: List[DifyNode]) -> List[DifyEdge]:
        ...
        # 3. 转换条件边 (使用 Code Node + If-Else 组合)
