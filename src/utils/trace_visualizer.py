...
"""
    ...
    for step in trace.steps:
        status_icon = "✅" if step.step_type.value == "success" else "❌" if step.step_type.value == "failed" else "⏭️"
...
"""
    ...
    html += """
        <div class="footer">
            Generated by Agent Zero Simulator
        </div>
    </div>
</body>
</html>
...
"""
    ...
    if trace.issues:
        ...
        for i, issue in enumerate(trace.issues, 1):
            summary += f"  {i}. [{issue.severity.upper()}] {issue.issue_type}: {issue.description}\n"
            if issue.suggestion:
